...
def easeInOutQuad(t):
    ...
    Fórmula: t²(3-2t) - ease-in-out sigmoide"""
    t = max(0.0, min(1.0, t))
...
def get_neighbors(pixel, all_pixels, max_dist=5):
    ...
    for px, py in all_pixels:
        ...
        if 2 <= dist <= max_dist:  # Distância mínima de 2px para evitar adjacência
...
def group_into_strokes(pixels, max_distance=5):
    ...
    Retorna lista de strokes, cada stroke é uma lista ordenada de pixels."""
    if not pixels:
        return []
    ...
    for pixel in pixel_list:
        ...
        # Ordenar pixels do stroke por proximidade espacial
        if len(stroke) > 3:
...
def order_stroke_pixels(pixels):
    ...
    while remaining:
        ...
        # Mover o pixel mais próximo para ordered
...
def draw_hand_on_canvas(canvas, hand_img, x, y, hand_mask_inv, hand_ht, hand_wd, canvas_ht, canvas_wd):
    """Versão simplificada da draw_hand_on_img para o novo algoritmo"""
    ...
    # Calcular offset na imagem da mão
    ...
    # Aplicar máscara e desenhar mão
    ...
    # Combinar mão com canvas usando máscara
...
def generate_sketch_video(
    ...
    """
    NOVO: Human Speedpaint - Objetos individuais + easing natural + stroke grouping
    
    Args:
        image_path: Caminho da imagem
        split_len: Parâmetro mantido para compatibilidade (não usado no novo algoritmo)
        frame_rate: FPS do vídeo
        skip_rate: Parâmetro mantido para compatibilidade (não usado no novo algoritmo)
    ...
    try:
        ...
        # Resize inteligente (mantém proporção, limita a 1920x1080)
        ...
        # Processar imagem com Canny (melhor para detectar contornos)
        img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        edges = cv2.Canny(img_gray, 50, 150)  # MELHOR: Canny ao invés de threshold
        ...
        # 2. NOVA ETAPA: Connected Components → OBJETOS individuais
        progress(0.15, desc="🎯 Identificando objetos individuais...")
        
        num_labels, labels = cv2.connectedComponents(edges)
        
        # Filtrar objetos pequenos (ruído)
        object_list = []
        for label_id in range(1, num_labels):  # Pular background (0)
            component_mask = (labels == label_id)
            size = np.sum(component_mask)
            if size > 50:  # Mínimo 50 pixels
                # Central da massa do objeto
                ...
                object_list.append({
                    ...
                    'center': center,
                    'size': size,
                    'pixels': list(zip(xs, ys))  # Pixels do objeto
                })
        
        # Ordenar objetos por importância (tamanho + posição)
        ...
        progress(0.2, desc=f"✏️ Ordenando {total_objects} objetos...")
        ...
        # 4. ANIMAÇÃO PRINCIPAL com EASING
        frames_per_object = max(10, int(frame_rate * 0.3))  # ~0.3s por objeto
        ...
        for obj_idx, obj in enumerate(object_list):
            ...
            n_pixels = len(obj_pixels)
            
            # Dividir objeto em strokes conectados (agrupamento espacial)
            ...
            for stroke_idx, stroke in enumerate(strokes):
                ...
                # EASING: curva Bézier para movimento natural
                frames_for_stroke = max(5, stroke_len // 10)
                
                for frame_in_stroke in range(frames_for_stroke):
                    t = frame_in_stroke / frames_for_stroke  # 0 a 1
                    
                    # Easing: easeInOutQuad (lento-início, rápido-meio, lento-fim)
                    ...
                    # MÃO na posição atual do stroke
                    if drawn_up_to > 0:
            ...
            # Transição suave entre objetos (frames de pausa)
            for pause_frame in range(3):
                video_object.write(canvas)
        
        # === FASE 2: COLORIZAÇÃO POR REGIÕES (se modo selecionado) ===
        if draw_mode == "Contornos + Colorização":
            progress(0.8, desc="🎨 Adicionando cores finais...")
            
            # Simples: adicionar imagem original colorida no final
            # (poderia ser expandido para colorização progressiva no futuro)
            for _ in range(frame_rate * 2):  # 2 segundos de transição
                video_object.write(img)
        
        # 5. Imagem final colorida
        ...
        return video_path, f"Vídeo com movimento NATURAL humano gerado em {duration:.1f}s!"
...
def generate_sketch_video_single(
    ...
    """
    VERSÃO HUMAN SPEEDPAINT: Objetos individuais + easing natural + stroke grouping
    Versão otimizada para batch processing
    """
    try:
        # 1. Carregar e pré-processar imagem
        img = cv2.imread(image_path)
        ...
        frames_per_object = max(8, int(frame_rate * 0.25))  # ~0.25s por objeto
